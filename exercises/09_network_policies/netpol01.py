...
def verify():
    ...
    assert spec.get("podSelector") == {}, "podSelector must be an empty dict {} to match all pods in namespace"
    assert set(spec.get("policyTypes", [])) == {"Ingress", "Egress"}, "policyTypes must include both 'Ingress' and 'Egress'"
    assert "ingress" not in spec or len(spec.get("ingress", [])) == 0, "Default deny must not define allow ingress rules"
    assert "egress" not in spec or len(spec.get("egress", [])) == 0, "Default deny must not define allow egress rules"
