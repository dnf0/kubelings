...
without masking other files.

In the multi-document manifest below:
1. Complete the ConfigMap 'nginx-config' with data key 'default.conf'.
2. In Pod 'nginx-configured':
   - Define a volume named 'config-vol' pointing to configMap 'nginx-config'.
   - In container 'nginx-server', mount 'config-vol' at '/etc/nginx/conf.d/default.conf'
