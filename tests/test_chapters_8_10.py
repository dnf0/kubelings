...
EXPECTED_EXERCISES = {
    "08_security_rbac": [f"rbac0{i}.py" for i in range(1, 6)],
    "09_network_policies": [f"netpol0{i}.py" for i in range(1, 5)],
    "10_lifecycle_probes": [f"health0{i}.py" for i in range(1, 5)],
}


def get_chapter_files(base_dir: str):
    """Collect all python files for chapters 8 to 10."""
    files = []
    base = Path(base_dir)
