"""Tests for Chapters 8 to 10 Curriculum & Reference Solutions."""
...
EXPECTED_EXERCISES = {
    "08_security_rbac": [f"rbac0{i}.yaml" for i in range(1, 6)],
    "09_network_policies": [f"netpol0{i}.yaml" for i in range(1, 5)],
    "10_lifecycle_probes": [f"health0{i}.yaml" for i in range(1, 5)],
}


def get_chapter_files(base_dir: str):
    """Collect all YAML files for chapters 8 to 10."""
    files = []
    base = Path(base_dir)
...
def test_manifest_matches_chapters_8_to_10_files():
    """Verify manifest definitions match exercise files on disk."""
    manifest = get_manifest()

    for ch in manifest.chapters[7:10]:
